...
import { PanelBody, TextareaControl, ToggleControl, ToolbarButton, ExternalLink, FocalPointPicker } from '@wordpress/components';
...
function MediaTextEdit(_ref2) {
  ...
  var image = useSelect(function (select) {
    return mediaId && isSelected ? select(coreStore).getMedia(mediaId) : null;
  }, [isSelected, mediaId]);
  var refMediaContainer = useRef();

  var imperativeFocalPointPreview = function imperativeFocalPointPreview(value) {
    var style = refMediaContainer.current.resizable.style;
    var x = value.x,
        y = value.y;
    style.backgroundPosition = "".concat(x * 100, "% ").concat(y * 100, "%");
  };
