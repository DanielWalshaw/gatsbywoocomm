...
function getHeadingsFromHeadingElements(headingElements) {
  return (0, _toConsumableArray2.default)(headingElements).map(function (heading) {
    return {
      ...
      anchor: heading.hasAttribute('id') ? "#".concat(heading.id) : '',
...
function getHeadingsFromContent(content) {
  ...
  var tempPostContentDOM = document.createElement('div');
  ...
  var _iterator = _createForOfIteratorHelper(tempPostContentDOM.querySelectorAll('template')),
      _step;
  ...
  var headingElements = tempPostContentDOM.querySelectorAll('h1:not(:empty), h2:not(:empty), h3:not(:empty), h4:not(:empty), h5:not(:empty), h6:not(:empty)');
...
function linearToNestedHeadingList(headingList) {
  ...
  var nestedHeadingList = [];
  headingList.forEach(function (heading, key) {
    if (heading.content === '') {
      return;
    ...
    if (heading.level === headingList[0].level) {
      ...
      if (headingList[key + 1] !== undefined && headingList[key + 1].level > heading.level) {
        ...
        var endOfSlice = headingList.length;

        for (var i = key + 1; i < headingList.length; i++) {
